...
class ReplaceAgentBlock(unittest.TestCase):
    def test_append_then_replace_is_idempotent_and_localized(self):
        c0 = ""
        c1 = wb.replace_agent_block(c0, "claude-101", "Current State: A")
        self.assertIn("<!-- hm:session=claude-101 -->", c1)
        self.assertIn("Current State: A", c1)
        # Replacing a session's block again leaves a single block (no duplication).
        c2 = wb.replace_agent_block(c1, "claude-101", "Current State: B")
        self.assertEqual(c2.count("<!-- hm:session=claude-101 -->"), 1)
        self.assertIn("Current State: B", c2)
        self.assertNotIn("Current State: A", c2)
        # Same body again → byte-identical (idempotent).
        self.assertEqual(c2, wb.replace_agent_block(c2, "claude-101", "Current State: B"))

    def test_unfenced_human_text_is_sacred(self):
        human = "# HUMAN_MEMORY.md\n\nhand-written notes the agent must not eat\n"
        out = wb.replace_agent_block(human, "codex-202", "codex section")
        self.assertIn("hand-written notes the agent must not eat", out)
        self.assertIn("<!-- hm:session=codex-202 -->", out)

    def test_two_agents_coexist_and_dont_touch_each_other(self):
        c = wb.replace_agent_block("", "claude-101", "claude work")
        c = wb.replace_agent_block(c, "codex-202", "codex work")
        self.assertIn("claude work", c)
        self.assertIn("codex work", c)
        # Editing codex leaves claude's bytes untouched.
        c2 = wb.replace_agent_block(c, "codex-202", "codex work v2")
        self.assertIn("claude work", c2)
        self.assertIn("codex work v2", c2)

    def test_same_agent_multiple_sessions_get_distinct_blocks(self):
        c = wb.replace_agent_block("", "codex-101", "first codex")
        c = wb.replace_agent_block(c, "codex-202", "second codex")
        self.assertIn("first codex", c)
        self.assertIn("second codex", c)
        self.assertEqual(c.count("<!-- hm:session=codex-"), 2)

    def test_remove_block(self):
        c = wb.replace_agent_block("", "claude-101", "x")
        c = wb.replace_agent_block(c, "codex-202", "y")
        c = wb.replace_agent_block(c, "claude-101", None)
        self.assertNotIn("hm:session=claude-101", c)
        self.assertIn("hm:session=codex-202", c)
...
class ConcurrentWriters(unittest.TestCase):
    def test_disjoint_agents_no_lost_updates(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            for a in agents:
                self.assertEqual(final.count(f"<!-- hm:session={a} -->"), 1,
    ...
    def test_same_agent_concurrent_no_corruption(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            with multiprocessing.Pool(8) as pool:
                pool.map(_worker, [(str(path), "claude-101", 15) for _ in range(8)])
            final = path.read_text()
            self.assertEqual(final.count("<!-- hm:session=claude-101 -->"), 1)
            self.assertEqual(final.count("<!-- /hm:session=claude-101 -->"), 1)
